...
Sequence :: Sequence() : m_size(0), m_decSize(DEFAULT_MAX_ITEMS) {
	m_arr = new ItemType[DEFAULT_MAX_ITEMS];
}

Sequence :: Sequence(int size) : m_size(0), m_decSize(size) {
	m_arr = new ItemType[size];
}

Sequence :: Sequence(const Sequence& other) : m_size(other.m_size) {
	m_arr = new ItemType[m_size];
...
{
	delete[] m_arr; //square brackets to avoid undefined behaviour
}
...
void Sequence::swap(Sequence& other) {
	...
	//swap contents
	int temp = m_decSize;
	m_decSize = other.m_decSize;
	other.m_decSize = temp;

	//swap ItemType contents
	ItemType* tempP = other.m_arr;
	other.m_arr = m_arr;
	m_arr = tempP;
